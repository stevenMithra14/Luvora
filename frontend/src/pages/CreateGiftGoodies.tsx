    ...
    <div className="h-[calc(100vh-4.5rem)] flex flex-col justify-between py-2 px-4 sm:px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col justify-center my-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-center flex-1 my-auto w-full">
          ...
          <div className="w-full lg:col-span-1 space-y-4 max-h-none lg:max-h-[calc(100vh-9.5rem)] overflow-y-auto pr-1 pb-12 sm:pb-0">
            {/* Header */}
            <motion.div
            ...
            <div className="p-3.5 sm:p-8 rounded-3xl bg-[#d5be9f] border-4 border-[#bda282] shadow-2xl text-slate-900 font-mono relative overflow-hidden space-y-6 sm:space-y-8 max-w-full">
              {/* Cardboard Texture Shadow Overlay */}
              <div className="absolute inset-0 bg-gradient-to-b from-black/5 via-transparent to-black/10 pointer-events-none" />

              {/* 1. SHIPPING LABEL STICKER */}
              <div className="relative max-w-md mx-auto xl:mx-0 bg-white p-4 sm:p-5 rounded-2xl border-2 border-slate-900 shadow-xl transform -rotate-1 transition-transform hover:rotate-0 max-w-full">
                {/* Header Bar */}
                <div className="bg-black text-white p-2 text-center rounded-lg mb-3 sm:mb-4">
                  <h3 className="font-mono text-xs sm:text-base font-extrabold tracking-widest uppercase">
                    DIGITAL CARE PACKAGE
                  </h3>
                </div>
                ...
                <div className="space-y-3 font-mono text-xs sm:text-sm text-slate-900">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <span className="font-extrabold text-sm sm:text-base text-slate-950 font-serif tracking-wide block truncate max-w-[160px] sm:max-w-none">
                        {data.recipientName || 'steven'}
                      </span>
                    </div>

                    {/* Red Delivery Stamp */}
                    <div className="border-2 border-rose-600 text-rose-600 font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[9px] sm:text-[10px] uppercase tracking-tighter transform rotate-[-8deg] shadow-sm text-center shrink-0">
                  ...
                  {/* Barcode Graphic */}
                  <div className="pt-2 sm:pt-3 text-center border-t border-slate-900 overflow-hidden">
                    <div className="font-mono text-xs sm:text-xl tracking-[0.1em] sm:tracking-[0.25em] font-extrabold text-black select-none whitespace-nowrap overflow-hidden text-ellipsis">
                      |||||||||||||||||||||||||||||||||||||||
                    </div>
                    <span className="text-[9px] sm:text-[10px] font-mono tracking-wider sm:tracking-widest text-slate-700 block truncate">
              ...
              <div className="relative bg-[#fcfbfa] p-4 sm:p-8 rounded-2xl border border-amber-900/20 shadow-2xl space-y-5 sm:space-y-6 text-slate-900 font-mono max-w-full">
                {/* Header Section */}
                <div className="text-center space-y-1 relative">
                  <h3 className="text-xs sm:text-base font-extrabold tracking-wider sm:tracking-widest uppercase text-slate-950">
                    A LITTLE BOX OF GOODIES
                  </h3>
                  <p className="text-[10px] sm:text-[11px] text-slate-500 italic font-serif">
                    est. for sending a little care
                  </p>

                  {/* Red Stamp Badge */}
                  <div className="inline-block border-2 border-rose-600 text-rose-600 font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-xs uppercase tracking-wider sm:tracking-widest transform rotate-[-4deg] shadow-xs my-1.5 sm:my-2">
                    things to tuck inside
                  </div>

                  <div className="text-[10px] sm:text-[11px] font-bold tracking-wider sm:tracking-widest text-slate-700 pt-1 sm:pt-2">
                    *** SELECT ITEMS TO TUCK ***
                  </div>
                </div>

                <div className="border-b-2 border-slate-900 pb-1 flex justify-between text-[10px] sm:text-[11px] font-bold text-slate-600 uppercase">
                  <span>ITEM</span>
                  <span>ADD</span>
                </div>
              ...
              {data.goodies.length > 0 && (
                <div className="bg-[#f7f3ee] p-4 sm:p-5 rounded-2xl border-2 border-slate-900 shadow-xl space-y-4 font-mono text-slate-900 max-w-full">
                  <div className="flex items-center justify-between border-b border-slate-900 pb-2">
                    ...
                    <span className="text-[10px] sm:text-[11px] text-slate-600 hidden sm:inline">Drag/reorder or edit</span>
                  </div>
                    ...
                    {data.goodies.map((item, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <span className="h-7 w-7 rounded-lg bg-pink-100 text-pink-700 text-xs font-mono font-bold flex items-center justify-center border border-pink-300 shrink-0">
                            0{idx + 1}
                          </span>
                          <div className="min-w-0">
                            <h4 className="font-mono text-xs font-bold text-slate-950 capitalize flex items-center gap-1.5 truncate">
